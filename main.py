    ...
    def retrieve_customer_context(self, event: MessageAddedEvent):
        ...
        for strategy_type, namespace_template in self.namespaces.items():
            ...
            for record in records:
                ...
                if text:
                    memories_collected.append(f"[{strategy_type}] {text}")

        if memories_collected:
            context_header = "Customer Context:\n" + "\n".join(memories_collected)
            if isinstance(content, str):
                last_message["content"] = f"{context_header}\n\n{user_text}"
...
def search_knowledge_base(query: str) -> str:
    ...
    if not chunks:
        return "No relevant text content found in retrieval results."
...
# 100 points = $1.00 USD
# Max points redemption is 50% of order total
max_points_allowed = int((order_total * 0.5) * 100)
eligible_points = min(points_balance, max_points_allowed)
points_redeemed = (eligible_points // 500) * 500
...
async def invoke(payload, context=None):
    ...
    try:
        ...
        actor_id = payload.get("customer_id") or "anonymous_customer"
